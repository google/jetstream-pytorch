...
from jetstream_pt.environment import JetEngineEnvironmentData, JetEngineEnvironment, QuantizationConfig
...
flags.DEFINE_integer("override_max_cache_length", 
                     -1, "Size of cache, defaults to input + output length")
...
_llama2_7 = ModelInfo(
    llama_model.Transformer, 32, 32, 128)
_llama2_13 = ModelInfo(
    llama_model.Transformer, 40, 40, 128)
_llama2_70 = ModelInfo(
    llama_model.Transformer, 80, 8, 128)
_llama3_8 = ModelInfo(
    llama_model.Transformer, 32, 8, 128)
...
def _bf_dir(repo_id):
  return os.path.join(_model_dir(repo_id), 'converted_bfloat')

def _int_dir(repo_id):
  return os.path.join(_model_dir(repo_id), 'converted_int8')


def get_sharding_config(repo_id):
  #TODO
  return None
...
def construct_env_data_from_model_id(
  ...
  shard_on_batch = sharding_config is None
...
def hf_download(
  repo_id: str, dest_directory: str,
  hf_token: Optional[str] = None) -> None:
  os.makedirs(dest_directory, exist_ok=True)
  try:
      snapshot_download(repo_id, local_dir=dest_directory, 
                        local_dir_use_symlinks=False, token=hf_token,
                        allow_patterns=["*.safetensors", "*.json", "*.model"])
  except HTTPError as e:
      if e.response.status_code == 401:
          print("Please use huggingface-cli login to authenticate "
